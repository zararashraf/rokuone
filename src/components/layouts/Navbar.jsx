...
import { faMagnifyingGlassLocation, faChartSimple, faChartArea, faChartBar, faChartColumn, faChartGantt, faChartLine, faChartPie, faMagnifyingGlass, faRobot, faGlobe, faStar, faParagraph, faGlasses, faMagnifyingGlassChart, faListUl } from '@fortawesome/free-solid-svg-icons'
import { faMailchimp, faYoutube, faYoutubeSquare } from "@fortawesome/free-brands-svg-icons"
import { Link } from "react-router-dom"
import { useState } from "react";

const Navbar = () => {
  const [menuOpen, setMenuOpen] = useState('md:hidden')

  const seoItems = [
    { 'icon': faMagnifyingGlassLocation, 'text': 'Keyword Research Tool' }, 
    { 'icon': faChartArea, 'text': 'Bulk Keyword Data Tool' }, 
    { 'icon': faChartBar, 'text': 'Questions Explorer' }, 
    { 'icon': faChartColumn, 'text': 'Backlinks Checker' }, 
    { 'icon': faListUl, 'text': 'Backlinks List' }, 
    { 'icon': faChartGantt, 'text': 'High CPC Keywords' },
  ...
  const emailItems = [
    { 'icon': faChartSimple, 'text': 'Email Validation Tool' }, 
    { 'icon': faChartPie, 'text': 'Bulk Email Validation' }, 
  ...
  return (
    <nav className='bg-white border-gray-200 px-20 py-10 shadow-lg'>
      <div className='container flex flex-wrap-reverse justify-between items-center'>
        ...
        <img src={logo} className='mr-3 h-6 sm:h-9' /></Link>

        <button onClick={() => { 
                                  if (menuOpen == 'md:hidden') 
                                  { setMenuOpen('') } 
                                  else { setMenuOpen('md:hidden') }
                                }
                        } data-collapse-toggle='mobile-menu-2' type='button' className='inline-flex duration-500 items-center p-2 ml-1 text-sm text-gray-500 md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600' aria-controls='mobile-menu-2' aria-expanded='false'>
        ...
        <div className='hidden mr-0 justify-between items-center w-full md:flex md:w-auto md:order-1' id='mobile-menu-2'>
          <ul className='flex flex-col mt-4 md:flex-row md:space-x-4 md:mt-0 md:text-sm md:font-medium'>
            <li>
              <DropDown text='SEO' icon={faChartLine} options={seoItems} />
            </li>
            <li>
        ...
        {/* Responsive Navbar */}
        <div className={`md:flex lg:hidden xl:hidden 2xl:hidden ${menuOpen}`} id='mobile-menu-2'>
          <ul className='flex flex-col mt-4 md:text-sm md:font-medium'>
            <li>
              <DropDown text='SEO' icon={faChartLine} options={seoItems} />
            </li>
            <li>
